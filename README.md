...
### 3. Демонстрація для звіту

```bash
# 1. Створити фіче-гілку, поламати тест навмисно, відкрити PR
git checkout -b broken-pr
echo "assert False" >> tests/test_health.py
git commit -am "Break a test (for demo)"
git push -u origin broken-pr
# Відкрити PR на GitHub — побачите, що Test job червоний, кнопка merge заблокована

# 2. Створити чисту фіче-гілку, відкрити PR, дочекатись зелених перевірок, замерджити
git checkout main
git checkout -b feature/add-comment
echo "# new comment" >> app/__init__.py
git commit -am "Add a comment"
git push -u origin feature/add-comment
# Відкрити PR — побачите, що все зелене, merge можливий

# 3. Створити анотований тег → запустить Build + Deploy + Verify
git checkout main
git tag -a v1.0.0 -m "Release v1.0.0"
git push origin v1.0.0
# В Actions побачите весь пайплайн до Verify
```

## Демонстраційні артефакти для міні-звіту

| Потрібно показати | Де брати |
|---|---|
| ✅ PR, успішно злитий | URL мерджнутого PR (з зеленими галочками) |
| ❌ PR, не може бути злитий | URL PR з червоними тестами (merge button сірий) |
| 📜 Лог успішного deploy + verify | сторінка Actions → конкретний run → log Verify job |
| 📜 Лог успішного deploy + НЕуспішного verify | штучно поламати nginx на target node і запустити Verify окремо; або відкотити v1.0.0 і прокинути v1.0.1 у поламаному стані |
| 📊 Звіт про покриття коду | артефакт `coverage-html` з кожного успішного run Test job |

## Безпека (примітки)

- Self-hosted runner використовується в публічному репо тільки на час лабораторної. **Після здачі — зупинити або видалити runner VM**, видалити runner у Settings → Actions → Runners.
- Жодних паролів/токенів у репо немає. БД-пароль генерується скриптом і зберігається на target node.
- Runner має SSH-доступ лише до target node, не root. Sudo на target node обмежений конкретними командами (`/etc/sudoers.d/deploy`).

## Що ще можна покращити

- Slack/email-нотифікації при failed verify (опціонально в завданні).
- Окремий staging-енв перед production.
- Сповіщення в Telegram через `appleboy/telegram-action`.
